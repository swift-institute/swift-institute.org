...
**ManagedBuffer nesting constraint**: `ManagedBuffer` subclasses MUST be nested at the **same level** as the `~Copyable` generic parameter declaration. A `ManagedBuffer<Int, Element>` at a deeper nesting level fails. This is distinct from the general cross-file extension pattern — it applies specifically to `ManagedBuffer` class inheritance.
...
**Cross-references**: [API-IMPL-005], [API-IMPL-008], [MEM-COPY-006], [COPY-FIX-003]
...
**Cross-references**: [IMPL-060], [PATTERN-053], [PATTERN-027]
...
**Cross-references**: [PATTERN-026], [IMPL-064]
...
**Cross-references**: [PATTERN-026], [PATTERN-027], [IMPL-060]
...
Rules for correctly structuring code with `~Copyable` generic parameters. These address compiler constraints that produce confusing diagnostics when violated. Absorbed from the former copyable-remediation skill.
...
**Statement**: Every extension on a type with `~Copyable` parameters MUST include explicit `where Element: ~Copyable`, unless intentionally restricted to `Copyable` elements. This applies to methods, computed properties, typealiases, nested types, and extensions on **nested types** within a generic outer type.
...
```swift
extension Container {
    func operation() { }  // ❌ Only available for Copyable elements
}
```

**Nested type extensions** — constraint appears redundant but is required:
```swift
public enum Storage<Element: ~Copyable> {
    public final class Heap: ManagedBuffer<Header, Element> { }
}

// CORRECT — constraint required even on nested type
extension Storage.Heap where Element: ~Copyable {
    public struct Header { ... }
}

// INCORRECT — fails despite outer type having ~Copyable
extension Storage.Heap {
    public struct Header { ... }  // ❌ "type 'Element' does not conform to 'Copyable'"
}
```

**Cross-references**: [MEM-COPY-004], [MEM-COPY-006] Category 2, [PATTERN-022]
...
// ❌ Poisons Container's stored properties in the original file
```

**Module boundary alternative**: When same-file placement isn't viable, split into separate SPM modules. See [COPY-FIX-010].

**Cross-references**: [MEM-COPY-006] Categories 3 and 5, [COPY-FIX-010]

---

### [COPY-FIX-005] Protocol Conformance Strategy

**Statement**: `Sequence`/`Collection` conformances MUST be conditional on `Element: Copyable`. For ~Copyable element iteration, use custom protocols or `forEach` with borrowing closures.
...
**Status**: OPEN. Tracking: swiftlang/swift #86669.
...
**Statement**: Types with conditional `Copyable` conformance MUST implement CoW in all mutating operations when `Element: Copyable`. The `~Copyable` tier mutates directly; the `Copyable` tier adds uniqueness checks.

```swift
// ~Copyable tier: direct mutation
extension Stack where Element: ~Copyable {
    public mutating func push(_ element: consuming Element) {
        ensureCapacity(_storage.header + 1)
        // direct mutation — single owner guaranteed
    }
}

// Copyable tier: CoW check before mutation
extension Stack where Element: Copyable {
    mutating func makeUnique() {
        if !isKnownUniquelyReferenced(&_storage) {
            _storage = _storage.copy()
            unsafe (_cachedPtr = _storage._elementsPointer)  // CRITICAL: update cached pointers
        }
    }
...
// ❌ Makes the type Sendable for ALL element types
```

**Silent failure mode**: `@unchecked Sendable where Element: ~Copyable` compiles without warning but grants Sendable to ALL element types — including non-Sendable ones. Always use `where Element: Sendable`.
...
**Status**: OPEN. Reproduced in Swift 6.2.4. Tracking: swiftlang/swift #86652.

**Statement**: The compiler does not synthesize member destruction for `~Copyable` structs whose stored property chain includes `@_rawLayout`-backed types across package boundaries. Element deinitializers silently fail — this is a **memory leak**, not a compile error.
...
**Applied to**: 21 types across 9 packages wrapping `Storage<Element>.Inline<capacity>`.
...
**Statement**: When same-file conformance placement [COPY-FIX-004] isn't viable, split into separate SPM modules. Module boundaries prevent constraint propagation because each target compiles independently.
...
**Statement**: Before implementing a `~Copyable` or `~Escapable` change to an associated type or generic parameter, trace every associated type through every conformer and extension to predict where explicit constraints will be needed.
...
- **memory-safety** skill for [MEM-SAFE-010–014] unsafe operation patterns (formerly in advanced-patterns)
