...
<!--
---
title: Reflections Consolidation
version: 2.0.0
...
**Trigger**: User points to this file to initiate consolidation.

**Source**: `Reflection Entries.md` in the same directory.

**Action**: Execute the Consolidation Loop defined in the visible document sections below.
...
**You MUST**:
1. **Match target document patterns** - If the target uses `[API-XXX-NNN]` numbered requirements with Scope/Statement/Correct/Incorrect/Rationale/Cross-references, your integration MUST follow that pattern
2. **Expand terse insights** - A 3-sentence reflection may become a full requirement with code examples, rationale, and cross-references
3. **Synthesize with existing structure** - Weave insights into the document's existing organization; create new sections only when the insight doesn't fit existing ones
4. **Add concrete examples** - Every principle needs Correct/Incorrect code examples showing application
5. **Establish connections** - Add Cross-references to related requirements in other documents

**You MUST NOT**:
- Copy reflection text verbatim (even with voice changes)
- Append content without matching document structure
- Create orphaned content that doesn't connect to existing sections
- Leave insights abstract when concrete examples would clarify

The permanent documentation serves as authoritative reference for LLMs working on this codebase. Incomplete or pattern-breaking content degrades that utility.
